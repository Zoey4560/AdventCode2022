...
class Monkey:
    ...
    def __init__(self, _monkey_string: str, relief=False):
    ...
    def _get_monkey_type_and_op(op_str):
        if op_str[25:] == 'old':  # assumes that there's only `old * old` and not `old + old`
            return MonkeyType.SQ, lambda x: x * x
        else:
...
def solve(_in, rounds, relief):
    monkeys = []
    for monkey_string in _in.split('\n\n'):
        monkeys.append(Monkey(monkey_string, relief=relief))
